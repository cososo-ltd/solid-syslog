...
| SR 6.1 | Audit log accessibility | SL1+ | `SolidSyslog_Log` is non-blocking — `SolidSyslogNullBuffer` for immediate send (single-task), `SolidSyslogCircularBuffer` for portable threaded use with an injected `SolidSyslogMutex` (Posix / Windows / Null / caller-supplied RTOS primitive), `SolidSyslogPosixMessageQueueBuffer` where a POSIX message queue is preferred. `SolidSyslog_Service` performs the blocking I/O on the integrator's chosen thread, bounded per attempt by `SO_SNDTIMEO` (5 s — see [`SolidSyslogPosixTcpStream.c`](../Platform/Posix/Source/SolidSyslogPosixTcpStream.c) and [`SolidSyslogWinsockTcpStream.c`](../Platform/Windows/Source/SolidSyslogWinsockTcpStream.c)) | None |
...
- A buffer — `SolidSyslogNullBuffer` (single-task), `SolidSyslogCircularBuffer` with a `SolidSyslogMutex` (cross-platform threaded — Posix / Windows / your RTOS primitive), or `SolidSyslogPosixMessageQueueBuffer` (POSIX `mqueue`).
