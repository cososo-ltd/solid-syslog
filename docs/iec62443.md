...
|---|---|---|
| **CR 1.5** Authenticator management | A [TLS platform](tls.md) presents your client credential, so the collector authenticates the device. You wire a credentials source and point it at your certificate and key; the library asks it for the material when a connection is made, and tells it when that connection has ended | Supply the credential and protect it where it rests. File permissions, a secure element or a hardware security module all work, because the library takes the material rather than the storage |
| **CR 1.8** Public key infrastructure certificates | The same [TLS platform](tls.md) verifies the collector against trust anchors you supply - or against certificate fingerprints you pin, where there is no PKI to anchor to - and checks its identity against a name you declare | Enrolment stays with your PKI. Where your assessment needs revocation checking, configure it in your TLS library: SolidSyslog neither performs that check nor prevents it, and the reasoning is under [TLS obligations](tls.md) |
