...
device to the collector. What any TLS stream must do — protocol floor, peer
verification, endpoint identity, mutual TLS — is stated once under
[TLS obligations](tls.md), which also states why revocation checking is not among
them. Each backend's page records where it falls short today. Both controls have
