...
> one. Certification applies to systems in deployment, and CRA obligations fall on the
...
adversary a system is expected to withstand — how skilled, how well resourced, and how
specifically motivated — rather than by any list of features. The definitions are in
the standard, and you will need your own copy to assess against them.

A Security Level is assigned to a system in its deployment and its assessment, not to a
component, and no component is bound to a level. The same library appears in
deployments assessed at different levels. This page therefore states what SolidSyslog
provides against each control and leaves the level question where the standard puts it:
with your own risk assessment. See [Building up the protection you
need](hardening-path.md) for the integration path and the question behind each step
along it.
