...
| [4.2.1](https://www.rfc-editor.org/rfc/rfc5425.html#section-4.2.1) | Certificate-based authentication - client | Supported | A client certificate and its key are optional configuration on the credentials source a TLS stream is wired to, presented only when both are given. A partial pair, a mismatched pair, and a credential the library will not take are reported rather than silently ignored, and delivery continues server-authenticated - see [Report a client credential that will not be presented](tls.md#report-a-client-credential-that-will-not-be-presented) |
...
| [5.2](https://www.rfc-editor.org/rfc/rfc5425.html#section-5.2) | Path validation, and authorised peers specifiable by host name | Supported | Peer verification chains to the trust anchors the caller supplies, and the declared peer identity is matched against the certificate: `dNSName` entries, the Common Name only where the certificate carries no subject alternative name, a wildcard only as the whole left-most label, and an address literal against `iPAddress` entries - see [Treat endpoint identity as declared](tls.md#treat-endpoint-identity-as-declared-not-assumed). RFC 9525 forbids the Common Name; the contract says how a deployment gets that stricter rule |
