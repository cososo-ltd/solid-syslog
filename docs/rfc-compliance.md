...
| [7.2.2](https://www.rfc-editor.org/rfc/rfc5424.html#section-7.2.2) | enterpriseId — MUST be an IANA-registered private enterprise number | Supported | Bounded at 64 decoded bytes, which is a library limit rather than an RFC one — §7.2.2 states no length. The library registers no enterprise number of its own and does not check the value's form: which number is yours, and that it is registered with IANA, is the integrator's |
