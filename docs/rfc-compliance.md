...
| [RFC 9662 §6](https://www.rfc-editor.org/rfc/rfc9662.html#section-6) | Early data (0-RTT) MUST NOT be used | Supported | RFC 9662 forbids it because syslog has no replay protection and early data has none between connections. The contract's [Do not send early data](tls.md#do-not-send-early-data) obligation binds every TLS `Stream`, shipped or caller-supplied |
...
| [4.2.1](https://www.rfc-editor.org/rfc/rfc5425.html#section-4.2.1) | Certificate-based authentication - client | Supported | A client credential is optional configuration on the credentials source a TLS stream is wired to, and is presented only when both certificate and key are given; §4.2.1 asks no more of a client. What is reported when half of one is given, and why delivery continues, is under [Report a client credential that will not be presented](tls.md#report-a-client-credential-that-will-not-be-presented) |
...
| [4.2.3](https://www.rfc-editor.org/rfc/rfc5425.html#section-4.2.3) | Administrators may select the cryptographic level | Supported | A cipher policy is asked for per connection through the `Stream`'s profile and applied before the handshake, so it binds the connection actually negotiated and a change takes effect on the next one. A policy that selects no suite fails `Open` rather than falling back, so one matching nothing is reported instead of silently ignored. What a given backend can express, and in how many lists, is on its own page |
| [4.2.3](https://www.rfc-editor.org/rfc/rfc5425.html#section-4.2.3) | A resumed session's security parameters SHOULD be checked against the requirements of the session being requested | Supported | The contract requires the check of any `Stream` that resumes, and a `Stream` that never resumes meets it by construction - see [Do not resume a session under weaker terms](tls.md#do-not-resume-a-session-under-weaker-terms) |
...
| [4.4](https://www.rfc-editor.org/rfc/rfc5425.html#section-4.4) | Close a connection not expected to carry more messages, sending `close_notify` first | Supported | Close sends `close_notify` before tearing the connection down, once and without delaying the close. A sender holds one connection open while it has records to deliver, which is the case §4.4 does not ask it to close; when it does close, and how an integrator bounds a long-lived connection, is under [A connection is long-lived](tls.md#a-connection-is-long-lived-and-the-integrator-bounds-it). §4.4's remaining sentences bind the receiver |
...
| [5.2](https://www.rfc-editor.org/rfc/rfc5425.html#section-5.2) | Path validation, and authorised peers specifiable by host name | Supported | Peer verification chains to the trust anchors the caller supplies, and the declared identity is matched against the certificate as §5.2 sets out - `dNSName` first, the Common Name as a fallback, a wildcard only as the left-most label. §5.2 also asks that the Common Name be supported, which RFC 9525 §4.1 later forbade; how the contract resolves that is under [Treat endpoint identity as declared](tls.md#treat-endpoint-identity-as-declared-not-assumed) |
...
|  [3.2](https://www.rfc-editor.org/rfc/rfc5426.html#section-3.2) | Avoid IP fragmentation (respect MTU) | Supported | The [Datagram](api/structSolidSyslogDatagram.md) contract obliges an implementor to report the path's largest payload and never guess high, and the sender trims and retries a record the path could not carry - whether the platform names it as oversize or only as a failure. Where no path MTU is available the fallback is what §3.2 calls the safest assumption for the family, 480 octets for IPv4 and 1180 for IPv6. The default message cap is itself 480, so nothing a default build emits can exceed it |
