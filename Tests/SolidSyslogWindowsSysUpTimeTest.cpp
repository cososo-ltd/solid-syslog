...
{
    fakeTickCount = 0;
    UNSIGNED_LONGS_EQUAL(0, SolidSyslogWindows_GetSysUpTime());
}
...
{
    fakeTickCount = 1000;
    UNSIGNED_LONGS_EQUAL(100, SolidSyslogWindows_GetSysUpTime());
}
...
TEST(SolidSyslogWindowsSysUpTime, MaxUint32Boundary)
{
    ...
    UNSIGNED_LONGS_EQUAL(UINT32_MAX, SolidSyslogWindows_GetSysUpTime());
}
