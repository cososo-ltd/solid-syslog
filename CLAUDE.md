...
| `SolidSyslogPosixMessageQueueBuffer.h` | System setup code using POSIX message queue buffer | `SolidSyslogPosixMessageQueueBuffer_Create(maxMessageSize, maxMessages)`, `_Destroy(base)`. Instance struct lives in a library-internal static pool (E11). The queue name derives from the process ID, so the default pool size of 1 is the supported configuration in a single process. Pool-exhaustion fallback is the shared `SolidSyslogNullBuffer`. |
